...
void ServerConnection::flush()
{
	...
	if (written > 0) {
		if (written < fWriteBufferLength) {
			/* Flushed part of the buffer, copy it back. */
			memcpy(fWriteBuffer, fWriteBuffer + written,
				fWriteBufferLength - written);
			fWriteBufferLength -= written;
		} else {
			/* Flushed the buffer completely. */
			fWriteBufferLength = 0;
		}
	}
}
...
void ServerConnection::processMessage()
{
	...
	if (messageClass == M_REQUEST) {
		...
		switch (messageId) {
			case CREATE_WINDOW:
				if (atom == NULL) {
					...
					if (atom == NULL || atom->getType() != ATOM_SERVER_WINDOW) {
						lprintf("create window called on non-window\n");
						return;
					}
			...
			case GET_BOUNDS: {
				if (atom == NULL || atom->getType() != ATOM_SERVER_WINDOW) {
					lprintf("get bounds called on non-window\n");
					return;
				}
			...
			default:
				lprintf("unknown request type %d\n", messageId);
		}
		...
		switch (messageId) {
			...
			case BEGIN_PAINT: {
				if (atom == NULL || atom->getType() != ATOM_SERVER_WINDOW) {
					lprintf("begin paint called on non-window\n");
					return;
				}
			...
			default:
				lprintf("Unknown command id %d\n", messageId);
		}
	} else {
		lprintf("Unknown message class %d\n", messageClass);
	}
}
